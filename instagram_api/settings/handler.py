...
class StorageHandler:
    ...
    def __init__(self, storage: StorageInterface, storage_config: dict, callbacks: list = None):
        callbacks = callbacks or {}

        if not isinstance(storage, StorageInterface):
            raise SettingsException('You must provide an instance of a StorageInterface class', response={})

        if not isinstance(storage_config, dict):
            raise SettingsException('The storage config must be a dict instance', response={})
    ...
    def move_user(self, old_username: str, new_username: str):
        ...
        if self._username in [old_username, new_username]:
            raise SettingsException('Attempted to move settings to/from the currently active user.', response={})
    ...
    def delete_user(self, username: str):
        ...
        if username == self._username:
            raise SettingsException('Attempted to delete the currently active user.', response={})
    ...
    def get(self, key: str):
        ...
        if key not in self.PERSISTENT_KEYS:
            raise SettingsException(f'The settings key "{key}" is not a valid persistent key name.', response={})

        key = self._user_settings.get(key, None)
    ...
    def set(self, key: str, value):
        ...
        if value is None:
            raise SettingsException(f'Illegal attempt to store null value in settings storage', response={})
    ...
    def set_cookies(self, data):
        ...
        self._storage.save_user_cookies(data)

    def _trigger_callback(self, callback_name: str):
        if callback_name not in self.SUPPORTED_CALLBACKS:
            raise SettingsException(f'The string "{callback_name}" is not a valid callback name.', response={})
    ...
    def _validate_empty_value(self, value):
        if not isinstance(value, str) or not value:
            raise SettingsException('Parameter must be non-empty string', response={})
    ...
    def _validate_user_is_active(self):
        if self._username is None:
            raise SettingsException('Called user-related function before setting the current storage user.', response={})
